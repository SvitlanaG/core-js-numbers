...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const cosine = product / (magnitude1 * magnitude2);
...
function toNumber(value, def) {
  ...
  return Number.isNaN(tempValue) ? def : Number(value);
}
...
function getSumOfDigits(num) {
  const tempStringValue = num.toString();
  let sum = 0;
  for (let i = 0; i < tempStringValue.length; i += 1) {
    sum += parseInt(tempStringValue[i], 10);
  }
  return sum;
}
...
function getSumOfNumbers(x1, x2, x3) {
  const numbersArr = [x1, x2, x3];
  const result = numbersArr.reduce((sum, num) => sum + num, 0);
